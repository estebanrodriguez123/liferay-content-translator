					...
					<c:choose>
						<c:when test="<%= Validator.isNull(toLanguageId) %>">
							...
							<c:if test="<%= (article != null) && !article.isNew() %>">
								...
								<!-- Rivet customization start -->
								<%
								String taglibEditURL = "javascript:Liferay.Util.openWindow({cache: false, dialog:{width: 350, height: 450}, id: '" + renderResponse.getNamespace() + "automatic-translate', title: '" + HtmlUtil.escapeJS(LanguageUtil.get(pageContext, "web-content-translation")) + "', uri: '" + HtmlUtil.escapeJS(translateArticleRenderPopUpURL) + "'});"; 
								%>
								<span class="lfr-translation-manager-add-menu">
								    <a class="btn" href="<%= taglibEditURL%>">
									    <liferay-ui:icon
	                                         image='add'
	                                         message='<%= LanguageUtil.get(pageContext, "add-automatic-translation") %>'
	                                     />
	                                     <span class="lfr-icon-menu-text"><%= LanguageUtil.get(pageContext, "add-automatic-translation") %></span>
                                     </a>
                                </span>
                                <!-- Rivet customization end -->
